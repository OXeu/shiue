...
- 第一版支持文章页的昵称 + 纯文本留言（最多 2,000 字符），评论记录不保存邮箱、IP 或头像，不提供附件、Markdown、嵌套回复或邮件回复通知。Vercel 限流仍会使用请求 IP，基础设施可能保留访问日志。
...
## 2. 配置 Vercel Firewall 限流

在项目 Firewall 中创建两条 **`@vercel/firewall`** 类型的限流规则，并发布规则变更：

| 环境变量 | Rate Limit ID 示例 | 起始阈值示例 |
| --- | --- | --- |
| `COMMENTS_RATE_LIMIT_ID` | `blog-comments` | 每 IP 每小时 5 次，提交邮件与确认发布共用 |
| `COMMENTS_CHALLENGE_RATE_LIMIT_ID` | `blog-comment-challenges` | 每 IP 每小时 20 次，独立限制挑战签发 |

挑战和邮件使用独立桶，正常提交不会重复占用邮件额度。挑战限流 ID 缺失时不会回退或放行。阈值需结合实际留言量、共享 IP 的影响调整；Vercel SDK 计数按区域跟踪，不是跨区域的严格全局配额。无需为读评论设置规则，因为评论是静态 HTML。

规则缺失、SDK 错误或限流服务不可用时接口拒绝签发挑战/发送邮件，避免配置遗漏时形成邮件轰炸入口。另有同源校验、JSON 类型/大小限制、隐藏诱捕字段和重复点击保护；Origin 可以被脚本伪造，不能单独当作反机器人机制。

**PoW 和 SDK 限流都在 Function 内执行，不能阻止请求触发 Function，也不是 DDoS/费用硬上限。** 为防止“打爆接口”，还需在 Vercel Firewall 为 `/api/comments-*` 配置请求路径匹配的边缘限流/阻断规则，让高频流量在函数执行前被拦截，并关注 Vercel/Resend 用量与预算告警。可用规则和额度以账户计划为准，仓库不会自动创建或购买规则。

预览部署明确拒绝真实发信/发布；本地 Hugo 服务器不运行 Vercel Functions。浏览器测试模拟 API，后端测试模拟限流/邮件/GitHub；不要通过伪造生产环境变量绕过本地限流。
...
1. 读者填写昵称、评论并同意公开说明后，前端 POST 同一份评论参数到 `/api/comments-challenge`。服务端校验内容、文章白名单和挑战限流，再签发 32 字节随机挑战与 HMAC-SHA256 令牌，有效期 5 分钟。
...
3. 前端向 `/api/comments-submit` 发送原评论和 `proof: { token, nonce }`。服务端校验签名、版本、站点、期限、当前难度及规范化评论摘要，自己计算一次哈希验收；不信任客户端传回的难度/哈希/耗时。失败时不调用限流 SDK 或 Resend；成功后仍需通过邮件限流。
4. 令牌绑定评论编号、文章路径、昵称、正文和提交时间。改变任意这些字段不能复用原证明。挑战令牌的签名用途和密钥与审批签名完全分离，挑战不提供审批能力，也不包含明文评论。
5. 每次失败重试获取新挑战，但未改动的评论沿用编号和时间，因此邮件幂等键不变。无数据库版本**不保证证明单次消费**：相同请求在有效期内可重放，由 Resend 24 小时幂等窗口和邮件限流兜底；不使用不可靠的 Function 进程内集合假装全局防重放。
...
```bash
npm run check:comments                 # PoW、签名、篡改、过期、限流、邮件幂等、审批和临时 Git 并发测试
npm run check                         # 包括临时评论的静态渲染、XSS 转义与文章隔离检查
PLAYWRIGHT_MODULE=/path/to/playwright/index.mjs \
  SHIUE_TEST_URL=http://127.0.0.1:1313/ node scripts/check-comments.mjs
```

浏览器检查使用真实构建后的 Worker 计算并以 Node Crypto 验证证明，同时覆盖取消、超时、Worker 故障、不支持的浏览器、挑战限流及过期后保留草稿。所有自动测试都不发送真实邮件、不调用 GitHub dispatch、不触发生产部署；Git 测试只操作临时仓库。配置好外部服务并部署后，再手动提交一条留言完成真实验收：浏览器验证 → 收邮件 → 核对内容 → 确认批准 → GitHub Action 成功 → 仓库新增评论文件 → Vercel 部署成功 → 页面出现留言。

- **503**：检查配置、限流规则、`comment-pages.json` 是否随函数打包，以及是否误用 Preview 环境。不会降级为未限流发信。
...
参考：[Vercel Functions](https://vercel.com/docs/functions/runtimes/node-js)、[Vercel Rate Limiting SDK](https://vercel.com/docs/vercel-firewall/vercel-waf/rate-limiting-sdk)、[Resend Send Email](https://resend.com/docs/api-reference/emails/send-email)、[GitHub workflow dispatch](https://docs.github.com/en/rest/actions/workflows#create-a-workflow-dispatch-event)。
