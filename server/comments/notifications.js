...
function notificationEmail(claim, type, to, from) {
  ...
  const introduction = type === 'approval' ? '你的留言已通过审核，正在发布。' : `${c.name} 回复了你的留言，回复已通过审核，正在发布。`;
  const url = `${claim.url}#comment-${c.id}`;
  const note = '网页将在构建和部署成功后更新，可能需要稍等片刻。此邮件为自动通知，请勿直接回复。';
  return {
    from, to: [to], subject: `${title} · ${claim.title}`,
    text: `${introduction}\n文章：${claim.title}\n\n${c.message}\n\n查看留言：${url}\n\n${note}`,
...
// 必须由审批接口在 GitHub 接受发布任务后调用；每位收件人单独发送，互不暴露邮箱。
export async function sendCommentNotifications(claim, parent, env, fetchImpl, types = ['approval', 'reply']) {
  const results = await Promise.all(types.map(async type => {
    try {
      const to = type === 'approval' ? claim.comment.email : parent ? openCommentEmail(parent, commentSecret(env, 'email')) : undefined;
      if (!to || (type === 'reply' && to.toLowerCase() === claim.comment.email?.toLowerCase())) return { type, skipped: true };
      ...
      if (!response.ok) { await response.body?.cancel(); throw new Error('Email not accepted'); }
      if (!(await response.json()).id) throw new Error('Missing email receipt');
      return { type, sent: true };
    } catch { return { type, failed: true }; }
  }));
  return { sent: results.filter(result => result.sent).length, failed: results.filter(result => result.failed).map(result => result.type) };
}
