// 评论文件的路径约定与发布前校验（在 GitHub Action 中执行）。
//
// 每条评论是 content/post/<文章目录>/comments/<UUID>.json 的独立文件。

import { lstat, readFile } from 'node:fs/promises';
import path from 'node:path';
import { CommentError, UUID, validateStoredComment, validateCommentDirectory } from './core.js';

/** 评论文件路径：目录与编号都先过校验，防路径穿越。 */
...
/** 校验评论目录真实存在且为普通目录：拒绝符号链接重定向写入位置。 */
export async function checkCommentDirectory(root, directory) {
  ...
  for (const part of ['content', ...directory.split('/'), 'comments']) {
    ...
    const info = await lstat(current).catch(error => {
      // comments 目录允许不存在（首条评论），其余层级必须存在。
      if (error.code === 'ENOENT' && part === 'comments') return null;
      throw error;
    });
    if (info && !info.isDirectory()) throw new CommentError(400, '评论目录必须是文章内的普通目录。');
  }
  if (!(await lstat(path.join(root, 'content', directory, 'index.md'))).isFile()) {
    throw new CommentError(400, '文章不存在。');
  }
}

/**
 * 沿 parentId 链向上校验所有祖先留言：必须属于同一文章且无循环引用。
 * 中间祖先可能已被删除（ENOENT），允许跳过；直接回复对象必须存在。
 */
export async function checkCommentParent(root, directory, comment) {
  ...
  while (id) {
    ...
    if (parent.id !== id || parent.path !== comment.path) {
      throw new CommentError(400, '回复的留言不属于此文章。');
    }
    id = parent.parentId;
  }
}
