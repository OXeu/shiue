...
import { APPROVAL_TTL, CommentError, validateComment, verify } from '../server/comments/core.js';
...
export async function appendApprovedComment({ root, envelope, secret, repository, now = Date.now() }) {
  ...
  // 文件名只能由通过校验的 UUID 组成；评论内容永远不是路径或 shell 代码。
  const relative = `data/comments/${comment.id}.json`;
  ...
  try { await writeFile(target, text, { flag: 'wx' }); }
    ...
    if (await readFile(target, 'utf8') !== text) throw new CommentError(409, '同一评论编号已存在不同内容，拒绝覆盖。');
    created = false;
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result = await appendApprovedComment({ root: process.cwd(), envelope: event.inputs?.envelope, secret: process.env.COMMENTS_WORKFLOW_SECRET, repository: process.env.GITHUB_REPOSITORY });
