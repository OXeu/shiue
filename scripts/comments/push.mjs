// 评论推送（GitHub Action publish-comment.yml 的第二步）。
//
// 把 publish.mjs 写好的评论文件提交并推送到发布分支。推送冲突时重放：
// fetch → rebase，最多 5 次；绝不 force push。
...
export function pushComment({ file, branch, cwd = process.cwd() }) {
  ...
  // 不用全局 Actions concurrency：它只保留一个 pending job，会丢掉突发
  // 审批。每条评论独立文件，非快进时 rebase 到最新分支后重试。
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      ...
      // 同一审批重放时随机邮箱密文可能不同；只自动合并这一种冲突——
      // 冲突文件唯一、且公开内容与邮箱摘要完全一致时，直接采用远端版本。
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.error('推送评论失败，请检查分支权限或重新运行任务。');
    process.exitCode = 1;
  }
}
