// 评论发布（GitHub Action publish-comment.yml 的第一步）。
//
// 验开审批信封，把评论写入 content/<文章>/comments/<UUID>.json。
// 每条评论独立文件，天然支持并发审批；同一编号已存在相同内容时幂等
// 重放（返回 created: false），内容不同则拒绝覆盖。
...
export async function appendApprovedComment({ root, envelope, secret, repository, now = Date.now() }) {
  ...
  try {
    ...
    // 重放场景：同一编号已存在。公开内容与邮箱摘要一致才算同一条。
    if (!sameStoredComment(JSON.parse(await readFile(target, 'utf8')), comment)) {
      throw new CommentError(409, '同一评论编号已存在不同内容，拒绝覆盖。');
    }
    created = false;
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result = await appendApprovedComment({
      root: process.cwd(),
      envelope: event.inputs?.envelope,
      secret: commentSecret(process.env, 'workflow'),
      repository: process.env.GITHUB_REPOSITORY,
    });
    // 输出给后续步骤（push.mjs）使用的文件路径与编号。
    if (process.env.GITHUB_OUTPUT) {
      await appendFile(process.env.GITHUB_OUTPUT, `comment_file=${result.relative}\ncomment_id=${result.id}\n`);
    }
