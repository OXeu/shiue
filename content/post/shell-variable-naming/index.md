...
![IDE 中实现目录书签与切换功能的 Shell 代码](/images/8b08d6b3d7ecf174a54ec001.png)

谢师傅快速运行测试了一遍，一切都工作地很好，但是没有自动补全，于是谢师傅再次让GPT代劳写了一个自动补全函数![IDE 中为目录书签命令添加自动补全的 Shell 代码](/images/514722c2fdaea156f99e7773.png)
...
```
 ...
 谢师傅虽然对于痛失 Sonoma 感到难受，但是对于自己（GPT）搭建的脚本工作流十分满意，正当谢师傅满意的使用 `cdm` 切换时，他看到了一个奇怪的提示![终端提示 cdm 脚本找不到 cat 命令](/images/2bd5d408defd85b697b3913c.png)

 他看了一遍自己的 cdm 函数，cat只有第 8 行有使用，![cdm 函数中调用 cat 读取书签文件的代码](/images/8b08d6b3d7ecf174a54ec001.png)
...
![cdm 函数把书签内容存入局部变量 path 后执行 cd 的代码](/images/b5b331a73b7e549d7b7d35dd.png)

........
突然屑师傅想到 shell 中可执行文件是通过环境变量 PATH 中指定路径去寻找的，如果找不到说明很可能 PATH 变量被修改了......
![一只因快速移动而模糊的猫表情包](/images/58c5fea818dbf8672f6ad947.png)

但是哪里会把 PATH 给改了呢？这里只有个 path，而且还是局部变量，怎么想都不可能...等一下，sHeLl 是不是不区分大小写来着？
![终端分别输出 path 与 PATH 变量的测试](/images/6a2c96ba8d7b58f8499f442d.png)

不对啊，这明明是区分大小写的，屑师傅迷惑了，但他还是准备试下另外的变量![终端测试显示 zsh 中 path 与 PATH 数组相互关联](/images/3e8030f764d10f21200c19c1.png)

path 和 PATH 是一样的！于是屑师傅将 path 改成了 p，错误消失！
![IDE 中把局部变量 path 改名为 p 的 Shell 代码](/images/0a40ace8fa836df01b49b9cc.png)

屑师傅释怀的笑了，至于为什么同样的代码在 Sonoma 上不会出现问题，也许是 zsh 版本不一样修改了实现啥的，但是屑师傅已经不关心了![“他们看起来好快乐啊”的企鹅表情包](/images/61e857f05761a21b3c561f87.png)
